...
本目录提供了四种事件存储的实现：PostgreSQL、Redis、Nats JetStream和内存存储。每种实现都支持完整的EventStore接口。

## 注意事项

在设计事件的时候，必须要注意两个字段的设计，一个是`ID`另一个是`AggreateID`。

- `ID`：每个事件都有一个唯一的ID，用于标识该事件。
- `AggreateID`：每个事件都属于一个聚合根，聚合根是一个领域模型，它负责维护事件的状态。聚合根的ID就是`AggreateID`。

所以，事件中不能直接使用ID值来携带聚合根的ID，因为ID是事件的唯一标识，而聚合根的ID是事件流的标识。


## 实现对比

| 特性             | PostgreSQL   | Redis        | Nats JetStream | BadgerDB     | 内存存储      |
| ---------------- | ------------ | ------------ | -------------- | ------------ | ------------- |
| **存储类型**     | 关系型数据库 | 内存键值存储 | 消息流+KV存储  | 嵌入式KV存储 | 内存集合存储  |
| **持久化**       | ✅ 永久存储   | ⚠️ 可配置     | ✅ 永久存储     | ✅ 永久存储   | ❌ 不持久化    |
| **事务支持**     | ✅ 完整事务   | ❌            | ✅ 原子操作     | ✅ ACID事务   | ✅ 互斥锁保护  |
| **并发控制**     | ✅ 乐观锁     | ✅ 版本检查   | ✅ 版本检查     | ✅ 事务锁     | ✅ 版本检查    |
| **批量操作**     | ✅            | ✅            | ✅              | ✅            | ✅            |
| **分页查询**     | ✅            | ✅            | ✅              | ✅            | ✅            |
| **时间范围查询** | ✅            | ✅            | ✅              | ✅            | ✅            |
| **快照支持**     | ✅            | ✅            | ✅              | ✅            | ✅            |
| **性能**         | 中等         | 高           | 高             | 非常高       | 极高          |
| **扩展性**       | 垂直扩展     | 水平扩展     | 水平扩展       | 垂直扩展     | 有限          |
| **网络传输**     | 有           | 有           | 有             | 无           | 无            |
| **嵌入式**       | ❌            | ❌            | ❌              | ✅            | ✅            |
...
err = eventStore.SaveEvents(ctx, "123", []event.Event{userCreated}, 0)
```
...
```go
import (
    "{{ .ProjectName }}/pkg/persistence/eventstore"
...
// 创建内存事件存储实例
eventStore := eventstore.NewMemoryEventStore(log)
...
func setupMemoryStore(t *testing.T) event.EventStore {
	log := logger.NewLogger()
	return eventstore.NewMemoryEventStore(log)
}
